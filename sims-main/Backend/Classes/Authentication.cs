...
namespace SIMS_Backend
{
    ...
    {
        public abstract AuthObj VerifyJWT(string token);
        public abstract bool IsAdmin(AuthObj auth);
    }
    ...
    public class Authentication : IAuthentication
    {
        ...
        private static string GetPubKey(bool force = false)
        {
            if (Environment.GetEnvironmentVariable("ecPublicKey") == null || force)
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        Console.WriteLine("No Auth Url defined or found!");
                        Environment.Exit(3);
                    }
    ...
    public class AuthObj
    {
        ...
        public AuthObj(bool tokenValid)
        {
            this.tokenValid = tokenValid;
        }

        public AuthObj(bool tokenValid, int userID, int roleID)
        {
            this.tokenValid = tokenValid;
            this.userID = userID;
            this.roleID = roleID;
